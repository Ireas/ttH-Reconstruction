...
# ==================================================
INPUT_PATH = "/media/ireas/Data/data/"
INPUT_FILE = "temp2.root"
OUTPUT_FILE = "temp.h5"


# main method 
def main():
	print(f"Converting {INPUT_PATH+INPUT_FILE} to {INPUT_PATH+OUTPUT_FILE}")
	
	# fill h5 file with root file 
	with uproot.open(INPUT_PATH+INPUT_FILE) as root_file:			
		# check if root_file has any entries
		if (len(root_file["prepared"].keys())==0):
			print(f"  Error: Empty reco keys")
			return
		
		# create new h5 file
		h5_file = h5py.File(INPUT_PATH+OUTPUT_FILE, 'w')
...
def fill_h5_from_root(h5_file, root_file):
	...
	number_of_jets = root_file['prepared/number_of_jets'].array()
	...
	root_energy = root_file['prepared/jet_e_NOSYS'].array()
	root_pt = root_file['prepared/jet_pt_NOSYS'].array()
	root_eta = root_file['prepared/jet_eta'].array()
	root_phi = root_file['prepared/jet_phi'].array()
	root_btag = root_file['prepared/jet_btag_85wp'].array()

	root_met_value = root_file['prepared/reco_met_value'].array()
	root_met_phi = root_file['prepared/reco_met_phi'].array()
	...
	# prepare root_files for fast access
	root_event_number = root_file['prepared/eventNumber'].array()

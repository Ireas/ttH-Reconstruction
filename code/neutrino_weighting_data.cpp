...
const string INPUT_FILE = "/media/ireas/Data/data/temp_injected.root";
const string OUTPUT_FILE = "/media/ireas/Data/data/temp_weighted.root";
...
int main(){
	...
	TChain rpreparedChain("prepared");
	TChain rPredictionChain("spanet");

	// append input file to chains
	rpreparedChain.Add(INPUT_FILE.c_str());
	rPredictionChain.Add(INPUT_FILE.c_str());


	//>> build befriended rDataFrames
	rPredictionChain.BuildIndex("eventNumber");  // just for security, use DSID too
	rpreparedChain.AddFriend(&rPredictionChain);

	auto rDataFrame = RDataFrame(rpreparedChain);

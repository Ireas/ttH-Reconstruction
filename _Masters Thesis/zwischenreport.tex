...
The second assumption posits that the $H$-boson is produced on-shell, with a SM mass of $M_H=125.20\pm0.11$\,GeV. The simulated MC samples were tested and the on-shell $H$-boson production dominates since this is a resonance process. Given the energy constraints, the Higgs boson cannot decay into two on-shell $W$-bosons. Therefore, one of the $W$-bosons must be off-shell. 

The third assumption specifies that the $W^*$-boson decaying leptonically is off-shell, marked by the asterisk. This is crucial because, without the on-shell constraint on the hadronic $W$-boson, it would be almost impossible to assign the jets to the $W$-boson, as any combination of jets could potentially reconstruct an off-shell $W^*$-boson with an indeterminate mass.

These assumptions constrain the kinematics of the event. Specifically, the energy of the lepton and neutrino is limited to approximately $\leq 50$ GeV, caused by the limited off-shell mass of the $W^*$-boson. Additionally, the two jets assigned to the on-shell $W$-boson are expected to have an invariant mass close to the SM $W$-boson mass value of $M_W=80.3692\pm0.0133$\,GeV \cite{pdg}.
...
To compare the performance of using certain variables to separate signal and background, the corresponding distributions are displayed together. Here, the signal is defined to only include semileptonic \ttHWW events while the background includes everything else. Naturally, the background contains the majority of all events. For a fair comparison, both signal and background are normalised to unity and binned using the same steps. The choice of variables is physically motivated by identifing measurable differences in the signal and background processes.
...
\end{align}
For each bin $i$, the variables $s_i$ and $b_i$ equal the normalised signal and background distributions, respectively. Since the separation power $\mathcal{S}$ is dependent on the binning, the absolute value has no physical meaning. However, when choosing the same binning, the separation power of several variables can be compared to identify the best performing ones. 

\begin{table}
    \centering
    \caption{Summary of the investigated variables sorted by their caluclated separation powers $\mathcal{S}$.}
    \begin{tabular}{l|c}
        Variable & $\mathcal{S}$\\
        \hline
        Total Jet Multiplicity & 21.9\,\% \\
        $t$ Assignment Probability & 21.2\,\% \\
        $W_\text{had}$ Assignment Probability & 9.0\,\% \\
        Missing Transverse Energy $E_T^\text{miss}$ & 3.0\,\% \\
        Lepton Energy $E_l$ & 1.2\,\% \\
...
For the missing transverse energy $E_T^\text{miss}$, the targeted semileptonic \ttHWW events peak approximately 10\,GeV lower than most other processes. In these events, the missing energy is expected to originate from the neutrino decay within the \HWW sub-system. Hence, $E_T^\text{miss}$ is limited by the energy of the leptonic $W^*$-boson decay. Furthermore, the reconstruction in this analysis assumes the leptonic $W^*$-boson to be off-shell which limits the maximum $E_T^\text{miss}$ energy to approximately $\leq50$\,GeV for an on-shell Higgs boson.Hence, the missing transverse energy can be used to identify events which fulfill the leptonic off-shell $W^*$-boson assumption. The separation power reaches $\mathcal{S}=3.0\,\%$ which corresponds to a similar sensitivity to signal and background.
...
% TODO: Include weight distribution
...
\begin{table}
    ...
    \begin{tabular}{l|r|r|r}
        & \multicolumn{1}{c|}{$SR$} & \multicolumn{1}{c|}{$CR_{t\bar{t}}$} & \multicolumn{1}{c}{$CR_{H\rightarrow bb}$}\\
        \hline
...
One $SR$ is defined to separate as much signal events from background contamination as possible. 
...
The $CR_{t\bar{t}}$ targets the dominating \ttbar process. Using the two subregions as defined in Tab.~\ref{TAB:EVENT_CLASSIFICATION:regions}, 99.93\,\% of events are \ttbar events.
...
% TODO: update region plot
\begin{figure}
    \centering
    \includegraphics[width=0.7\textwidth]{figures/event_selection/regions.png}
    \caption{Overview of the region definition. For visual clarity, the \ttbar events have been scaled down by a factor of 100.}
    \label{FIG:event_selection:regions}
\end{figure}

\chapter{Results}
In this chapter, the results of this study are summarised. First, the validation and performance of the truth matching script are stated in Section~\ref{results:truth_matching}. Sections~\ref{results:spanet} and \ref{results:neutrino_weighting} list the results from implementing \spanet and \NW, respectively. The final Section~\ref{results:data} includes the final results from combining all anaylsis steps and applying it on real data.
...
To better understand \spanets output, the predicted assignment for each jet is listed against the true parton. The resulting assignment matrix is shown in Fig.~\ref{fig:results:spanet:assignment_matrix}. Note that each column is normalised to unity. Moreover, no partial events are including which means that every parton has a jet assigned. 

The figure shows dominating diagonal terms, with over 80-90\,\% of events included. A diagonal matrix would correspond to a perfect prediction where every \spanet assignment equals the true assignment. The $(q_1,q_2)$ 2x2 submatricies for $t_1$, $t_2$ and $W_\text{had}$ show the predefined allowed permutations during learning. Hence, a off-diagonal term within these submatricies is considered as a correct assignment since the permutation does not change the underlying phyiscs. These outputs indicate a well-trained neural network.

\begin{figure}
    \centering  
    \includegraphics[width=0.7\textwidth]{figures/assignment_matrix/assignment_matrix.png}
    \caption{Overview of \spanet assignment matrix showing the predicted and true labels. Each column is normalised to unity. Only events with all partons matched are included.}
...
To validate the accuracy of the NW algorithm, the estimated solutions are compared to the true values of the MC samples. %Fig.~\ref{fig:results:neutrino_weighting:cherry} shows a distribution of the \NW output. 
Here, the \NW algorithm uses the truth information as input. Hence, effects due to simulated detector inefficiencies are ignored in the comparison. 
...
As an additional validation step, the assumed Higgs mass $m_H$ is varied. The used MC samples for validation uses a true mass of $m_H^\text{MC}=125$\,GeV. The \NW algorithm is tested by assuming the correct mass of $m_H=m_H^\text{MC}$ and a slightly off-set mass $m_H=125.20$\,GeV. Fig.~\ref{FIG:neutrino_weighting:validation_higgs_mass} shows the difference of the estimated values of $\eta_\nu$ and $m_W$ for both assumptions. The heatmaps clearly show the sensibility of the algorithm in respect to the assumed $H$-boson mass. Furthermore, for a correct assumption $m_H=m_H^\text{MC}$, the distribution peaks around ($\nu_\eta=0$,$m_W=0$\,GeV) as expected. Using a different $H$-boson mass significantly changes the estimated mass its decay particle $W_\text{lep}$. Hence, when using $m_H=125.20$\,GeV the estiamted mass $m_W$ is increased by around 200\,MeV in respect to the true value due to kinematic conservation laws. 

\begin{figure}
    \centering
    \includegraphics[width=0.46\textwidth]{figures/Neutrino Weighting/truth_2d_delta_mw_delta_nueta.png}\hspace{0.02\textwidth}
    \includegraphics[width=0.46\textwidth]{figures/Neutrino Weighting/truth_alt_2d_delta_mw_delta_nueta.png}
...
Furthermore, in Fig.~\ref{FIG:neutrino_weighting:true_and_reco} the same distribution is shown. Here the $H$-boson mass is not varied, but the input uses the reconstructed information, including simulated detector effects. Again, the predictions are centered around ($\nu_\eta=0$,$m_W=0$\,GeV). However, when using reconstructed information as input, the \NW estimations deviate stronger from the true values. These inaccuracies are expected due to less accurate inputs.

\begin{figure}
    \centering
    \includegraphics[width=0.46\textwidth]{figures/Neutrino Weighting/truth_2d_delta_mw_delta_nueta.png}\hspace{0.02\textwidth}
    \includegraphics[width=0.46\textwidth]{figures/Neutrino Weighting/reco_2d_delta_mw_delta_nueta.png}
    \caption{Comparison showing the \NW output difference when using (left) true informatio and (right) reconstructed information as input.}
    \label{FIG:neutrino_weighting:true_and_reco}
\end{figure}

\subsection*{Performance}
To evaluate the \NW algorithm, its output weights are analysed per process which are summarised in Tab.~\ref{tab:results:neutrino_weighting:counts}. Using the true values as input yields insight into the mathematical accuarcy of the reconstruction when ignoring detector effects and \spanet assignment efficiencies. Naturally, the \NW is almost 100\% accurate. When using the reconstructed information as input, the performance is more realistic due to the simulated detector inaccuracies. 

\begin{table}
    \centering
    \caption{List of reconstructable events by \NW split by different inputs. Events are counted as reconstructable when at least one solution was found. Note the different scales.}
    \begin{tabular}{l|c|c|c|}
        & true & reconstructed & reconstructed input\\
        & input & input &  \& optimisations\\
        \hline
        \ttHWWlvqq & XX.X & 0.11 & XX.X \\
        \ttHWW & XX.X & XX.X & XX.X \\
        \ttHbb & XX.X & XX.X & XX.X \\
        \ttHother & XX.X & XX.X & XX.X \\
        \ttbar & XX.X & XX.X & XX.X \\
        other & XX.X & XX.X & XX.X \\
    \end{tabular}
    \label{tab:results:neutrino_weighting:counts}
\end{table}
The distribution of weights is shown in Fig.~\ref{fig:results:neutrino_weighting:weight_distribution}. 

% TODO: Weight values

% TODO: Update weight destribution
\begin{figure}
    \centering  
    \includegraphics[width=0.7\textwidth]{figures/placeholder.png}
    \caption{Summary of the \NW output distribution split into different processes. Here, only events with at least one solution are included.}
    \label{fig:results:neutrino_weighting:weight_distribution}
\end{figure}


\section{Data}
\label{results:data}
% TODO: Merge of SPANet and Neutrino Weighting DATA
\subsection*{Regions}
...
\chapter{Discussion}
To conclude this thesis, a summary of the main results is given in Sec.~\ref{sec:discussion:summary}. This section will reiterate the main measurements with respect to the main goal of this analysis. Lastly, inspiration for future steps in this analysis are explained in Sec.~\ref{sec:discussion:future}.
...
This study only includes statistical uncertainties for the final results. Hence, systematic uncertainties due to detector calibrations, theoretical models, luminosity and other effects are not incoperated. In the future, this analysis should be complemented by introducing these systematic uncertainties.

Combining systematic and statistical uncertainties will yield more realistic uncertainties for the measurements. Moreover, varying the systematic uncertainties would show the sensitivity of the measurements with respect to the investigated uncertatinties. Hence, the reduction of the most sensitive uncertainties should then be prioritised to improve the results of this study. 
...
% TODO: Clean up Appendix
\chapter{Full Split PLots}

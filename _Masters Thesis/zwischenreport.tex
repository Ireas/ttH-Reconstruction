...
The targeted event topology is the Higgs-associated top quark pair production in which the Higgs boson decays into two $W$-bosons. The topology is a fully hadronic decaying \ttbar pair with an additional $H$-boson being radiated. The latter decays to two $W$-bosons which then further decay semi-leptonically, which means one $W$-boson decays hadronically into two quarks while the other decays leptonically into a lepton neutrino pair. In the following, these events will be referred to as semileptonic \ttHWW events. The corresponding Feynman diagram of the targeted topology can be seen in Figure~\ref{fig:feynman_tthww}. 

% TODO: change H propagator
\begin{figure}[t]
    \centering
    \includegraphics[width=.6\textwidth]{figures/feynman/ttH_labeled.png}
    \caption{Feynman diagram of the targeted \ttHWW event topology.}
    \label{fig:feynman_tthww}
\end{figure}

Due to the insufficient mass of the $H$-boson to create two on-shell $W$-bosons, one of the two $W$-bosons must be off-shell. This is denoted by the asterisk superscript $W^*$. The assumption that the on-shell $W$-boson is decaying hadronically simplifies its reconstruction since the two selected decay jets should match the SM $W$-boson mass as close as possible. Thus, it can be used to supress some of the main backgrounds (\ttbar + jets, $W$ + jets) due to the constrain on the decay jet properties. However, it also necessitates that the off-shell $W^*$-boson decays leptonically which complicates the leptonic reconstruction. This will be solved by introducing a \NW technique.

The rarity of the semileptonic \ttHWW events makes its measurement in the background difficult. Fig.~\ref{fig:theory:total_crosssections} depicts a summary of several event cross section measured by \atlas. It shows the dominating \ttbar background with significantly higher a cross section of ($829\pm16$)\,pb compared to \ttH which reaches ($0.670\pm0.142$)\,pb \cite{theory:cross_sections}. Hence, the \ttH events are three orders of magnitude more rare than \ttbar events. Other background processes such as $tW$, \ttZ and \ttW have cross sections higher or close to \ttH.

However, this study focuses on a particular $H$-boson decay mode. Hence, the branching ratio of the Higgs decay must be taken into account. For the previously stated SM $H$-boson mass $M_H$, the theoretical branching ratio of $H\rightarrow WW$ is the second highest at ($25.7\pm2.5$)\,\% \cite{theory:higgs_branching_ratio}. Furthermore, the $W$-boson decay ratios are ($67.41\pm0.27$)\,\% hadronically and ($32.58\pm0.47$)\,\% \cite{theory:w_branching_ratio}. Multiplying the \ttH cross section with these decay ratios and taking the $W$-permutations into account yields a final cross section of 0.079\,pb for the targeted semileptonic \ttHWW event topology. This results to approximately 10000 expected \ttbar events per semileptonic \ttHWW event. Taking other background processes into account, semileptonic \ttHWW events become even more difficult to measure in background.

\begin{figure}[t]
    \centering
    \includegraphics[width=.76\textwidth]{figures/cross_section/top_total_cross_sections.png}
    \caption{Summary of several $t$ quark related production cross section measurements. The theoretical expectations were calculated at least at NLO \cite{theory:cross_sections}.}
...
Furthermore, several technical optimisations such as hashmaps and bit-shifting operations are implemented to increase the processing speed. To implement bit-shifting, each jet gets an integer number assinged. Each parton corresponds to one of the bits in the integers bit-representation. Bit values of 1 mean the jet is potentially matched to the corresponding parton, while 0 means that the parton is not included withing the jets $\Delta R$ cone. Each parton has a fixxed position in the bit-representatin. Hence, checking a potential parton-jet match is done efficiently by bit-operations.
...
The targeted semileptonic \ttHWW event topology is introduced in Sec.~\ref{sec:theory:tthww}. Its final state consists of eight jets and a single lepton-neutrino pair. Six of these jets originate from the \ttbar decays, while the remaining two jets stem from the Higgs decay subsystem. Among the eight jets, exactly two are tagged as $b$-jets which orignitate from the $t$ quark decay $t \rightarrow Wb$. For the purposes of event reconstruction, three simplifying assumptions are made. 
...
The $t$ assignment proability generally is higher for the semileptonic and dileptonic \ttHWW events which peak at 0.7 and 0.6, respectively. \ttbar events score the lowest. Its peak is below 0.05. The 8+ jet limitation causes \ttbar events to have at least two additional jets measured. Hence, the additional possible jet choices reduce the probability of the correct jet assignment. The \ttHbb events also peak at 0.05 which is caused by the two additional $b$ quarks. These $b$ quarks from the $H$-boson decay can be misidentified as the $b$ quark from the $t$ quark decays. The average of both $t_1$ and $t_2$ assignment probability is $\Gamma=0.212$ which is the second highest separation power of all variables.  

Similarly, the $W_\text{had.}$ assignment probability also separates the different events topolgies. However, the distributions are not as clearly separated compared to the $t$ assignment probabilities. Here, \ttbar and \ttHbb peak at around 0.3 while the semileptonic \ttHWW peaks at 0.55. This variable reaches a separation power of $\Gamma=0.090$. Compared to $\Gamma$ of the $t$ assignment probability, this variable is less sensitive to signal. Still, both output probabilities are used to define the final event regions.
...
To separate signal and background, the overall phase space is subdivided in signal regions $S$ and control regions $C$. If defined properly, the signal region is expected to include a relatively high purity of the targeted semileptonic \ttHWW events while excluding as much of background contamination as possible. The control regions contain the remaining events with high background yields. Each control region targets one of multiple background processes. Only reconstructable events by the \NW algorithm are selected. Given the rarity of the \ttHWW process, it is important that the union of all regions covers the entire remaining phase space to avoid discarding events.

The signal and control regions are constructed to be non-overlapping, ensuring that the control regions remain orthogonal to the signal regions. Again, the choice of variables and the selected cuts for each region is driven by their respective separation power. The qualitative cut values are chosen by analysing their separation power. An overview of all regions is given in Tab.~\ref{TAB:EVENT_CLASSIFICATION:regions} with their relative yields in Tab.~\ref{TAB:EVENT_CLASSIFICATION:regions_yields}.

\begin{table}
    \centering
    \caption{Overview of the defined signal and control regions and their corresponding cuts. Both $C_{t\bar{t}}$ and $C_{t\bar{t}}'$ are included in the same control region.}
    \begin{tabular}{r|c|c|c|c}
        & $S$ & $C_{t\bar{t}}'$ & $C_{t\bar{t}}$ & $C_{H\rightarrow bb}$ \\
        \hline
        Total Jet Multiplicity & $\geq$8 & $\geq8$ & $<8$ & --- \\
        $b$-Jet Multiplicity & =2 & =2 & =2 & $\geq$3 \\
        $t$ Assignment Probability & $\geq0.85$ & $<0.85$ & --- & --- \\
        $W_\text{had}$ Assignment Probability & $\geq0.85$ & $<0.85$ & --- & --- \\
...
\begin{table}
    \centering
    \caption{Overview of the expected relative yields in each defined regions. The last row summarises the relative proportion of the region in respect to the full MC sampleset.}
    \begin{tabular}{l|r|r|r}
        & \multicolumn{1}{c|}{$S$} & \multicolumn{1}{c|}{$C_{t\bar{t}}$} & \multicolumn{1}{c}{$C_{H\rightarrow bb}$}\\
        \hline
        \ttbar      & 99.45\,\% & 99.93\,\% & 99.67\,\% \\
        \ttHbb      & 0.09\,\% & 0.02\,\% & 0.26\,\% \\
        \ttHcc      & 0.01\,\% & 0.00\,\% & 0.00\,\% \\
        \ttHtautau  & 0.02\,\% & 0.01\,\% & 0.01\,\% \\
        \ttHWWqqqq  & 0.08\,\% & 0.01\,\% & 0.01\,\% \\
        \rowcolor{highlighter!40}
        \ttHWWlvqq  & 0.28\,\% & 0.01\,\% & 0.02\,\% \\
        \ttHWWlvlv  & 0.01\,\% & 0.00\,\% & 0.00\,\% \\
        Other       & 0.04\,\% & 0.01\,\% & 0.01\,\% \\
        \hline
        Proportion of Total & 0.03\,\% & 77.92\,\% & 22.05\,\% \\
    \end{tabular}
    \label{TAB:EVENT_CLASSIFICATION:regions_yields}
\end{table}

\subsection*{Signal Region}
One signal region $S$ is defined to separate as much signal events from background contamination. 

The targeted topology expects 8 jets of which two jets are $b$-tagged. Hence, these variables are crucial for the region definition as summarised in TaTab.ble~\ref{TAB:EVENT_CLASSIFICATION:regions}. The total event multiplicity was varied. However, a lower limit decreases the purity significantly due to \ttbar events. A higher jet limit on the other hand decreases the total signal events drastically.

Furthermore, the \spanet assignment probability for each $t$ quark and $H$-boson are sensitve to the signal topology. Thus, a cut on these variables is applied which increases the signal purity by a factor of approximately 5. The cut-off value was also varied and the chosen value maximises the signal purity while retaining a reasonable amount of signal events.

As listed in Tab.~\ref{TAB:EVENT_CLASSIFICATION:regions_yields}, the signal region $S$ consists of 0.28\,\% of signal events which corresponds to 0.53\% of all signal events. This signal purity is mainly dominated by \ttbar events which have significant higher cross-section and similar kinematics.

\subsection*{Control Regions}
Two additional control regions $C_{t\bar{t}}$ and $C_{H\rightarrow bb}$ are defined to separate \ttbar and \ttHbb events, respectively. 

The $C_{t\bar{t}}$ control region targets the dominating \ttbar process. Using the two subregions as defined in Tab.~\ref{TAB:EVENT_CLASSIFICATION:regions}, 99.93\,\% of events are \ttbar events.

For the $C_{H\rightarrow bb}$ control region, the $b$-jet multiplicity is of particular interest. Since it is the only background process with a higher expected number of $b$-quarks in the final state, it can be separated by cutting events with more than two $b$-jets detected. This behaviour can also be seen in Fig.~\ref{FIG:separation_power_plots:jet_multiplicities}. This results in a region containing 0.26\,\% \ttHbb events as seen in Tab.~\ref{TAB:EVENT_CLASSIFICATION:regions_yields}.
...
The first matching scheme compares only the spatial separation between the potential jets and the targeted particle. The closest jet when calculating $\Delta R$ in respect to the parton is selected as the final match. The second approach utilises only $\Delta p_T$ of the potential jets and the targeted particle. Here, the closest pair in momentum space is selected as the final match. This scheme is motivated by the CP violating properties of the $W$-boson which should result in non-symmetric transverse momenta for the decay particles. The last scheme combines the information of both $\Delta R$ and $\Delta p_T$. For combined scheme, the $\Delta R$ was scaled to a range of [0,4] and $\Delta p_t$ to a range of approximately [0,10]. The upper bound one is not exact since there is no $p_T$ limit applied beforehand. Then, the pair that minimises the sum of both variables is chosen as the final match.
...
To train \spanet, several training sets were used and evaluated. The final model is trained on a MC set containing 2.5 million MC events composed of \ttbar and several \ttH events with additional \ttHWW events injected to improve the network performance. All events are preselected to have exactly one lepton and at least five detected jets.

Moreover, the architecture and other hyperparameters were optimised in a grid-search to find the best performing network. The final choice or parameters is listed in Tab.~\ref{tab:results:spanet:final_model_parameters}. The following results are based on this final model.

\begin{table}
    \centering
    \caption{Summary of the \spanets final architecture and hyperparameters. Parameters marked with * are not optimised.}
    \begin{tabular}{l|c}
        Epochs & 400 (early stop at 122)\\
        Batch Size & 2048\\
...
% TODO: short introduction neutrino weighting estimations (what, rückbezug wie)
...
% TODO: State the used sigma_x/sigma_y, city energy dependence resolution (worst case)
